/*
 * jsinit ADMIN TEMPLATE JAVASCRIPT
 * ======================================================================
...
For more efficient, simply called "jsinit.container".
...
!function ($) {
	...
	jsinit.window.on('load', function(){
		...
		jsinit.body.addClass('jsinit-ready');
	});
...
!function ($) {
	...
	jsinit.window.one('load', function(){
		if (jsinit.scrollTop.length && !jsinit.isMobile) {
			var isVisible = true;
			var offsetTop = 250;

			jsinit.window.scroll(function(){
				if (jsinit.window.scrollTop() > offsetTop && !isVisible) {
					jsinit.navbar.addClass('shadow');
					jsinit.scrollTop.addClass('in');
					isVisible = true;
				}else if (jsinit.window.scrollTop() < offsetTop && isVisible) {
					jsinit.navbar.removeClass('shadow');
					jsinit.scrollTop.removeClass('in');
					isVisible = false;
				}
			});

			jsinit.scrollTop.on('click', function(e){
				e.preventDefault();

				jsinit.bodyHtml.animate({
					scrollTop : 0
				}, 500);
			});
...
!function ($) {
	...
	methods = {
		'show' : function(el){
			...
			ovId = 'jsinit-overlay-' + uID() + uID()+"-" + uID(),
			panelOv = $('<div id="'+ ovId +'" class="panel-overlay"></div>');

			el.prop('disabled', true).data('jsinitOverlay',ovId);
		...
		'hide': function(el){
			var target = $(el.attr('data-target'));
			var boxLoad = $('#'+ el.data('jsinitOverlay'));
	...
	$.fn.jsinitOverlay = function(method){
		if (methods[method]){
...
!function ($) {
	...
	$.jsinitNoty = function(options){
		...
		focusElement = function(pos){
			jsinit.bodyHtml.animate({scrollTop: pos}, 300, function(){
				el.addClass('in');
			});
		},
		init = function(){
			if (opt.container === 'page') {
				if (!pageHolder) {
					...
					jsinit.contentContainer.prepend(pageHolder);
				};
				...
				if (!floatContainer[opt.floating.position]) {
					...
					jsinit.container.append(floatContainer[opt.floating.position]);
				}
...
!function ($) {
	...
	formElement = function(el){
		if (el.data('jsinit-check')) return
		else
		el.data('jsinit-check', true)
		...
		changed = function(){
			if(input.type == 'radio' && $groupInput.length) {
				$groupInput.each(function(){
					var $gi = $(this);
					if ($gi.hasClass('active')) $gi.trigger('jsinit.ch.unchecked');
			...
			if (input.checked) {
				el.addClass('active').trigger('jsinit.ch.checked');
			}else{
				el.removeClass('active').trigger('jsinit.ch.unchecked');
			}
		};
	...
	$.fn.jsinitCheck = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
	...
	jsinit.document.ready(function() {
		allFormEl = $('.form-checkbox, .form-radio');
		if(allFormEl.length) allFormEl.jsinitCheck();
	});

	jsinit.document.on('change', '.btn-file :file', function() {
		var input = $(this),
...
!function ($) {
		...
		$menulink.each(function(){
			...
			on('shown.bs.popover', function () {
				...
				offsetBottom 	= function(){
					if(jsinit.container.hasClass('mainnav-fixed')){
				...
				if( offsetTop > offsetBottom){
					...
					if(!jsinit.container.hasClass('navbar-fixed') && jsinit.mainNav.hasClass('affix-top')) offsetBottom -= 50;
					if(popoverHeight > offsetBottom){
						if(jsinit.container.hasClass('navbar-fixed') || jsinit.mainNav.hasClass('affix-top')) offsetBottom -= (elHeight + 5);
			...
			.on('click', function(){
				if(!jsinit.container.hasClass('mainnav-sm')) return;
	...
	updateSize = function(){
		...
		var sw = jsinit.container.width(), currentScreen;
		...
		if (screenCat != currentScreen){
			...
			jsinit.screenSize = currentScreen;

			if(jsinit.screenSize == 'sm' && jsinit.container.hasClass('mainnav-lg')){
				$.jsinitNav('collapse');
			}
		}
	},
	updateNav = function(e){
		...
		if (updateMethod == 'collapse' || jsinit.container.hasClass('mainnav-sm') ) {
			jsinit.container.removeClass('mainnav-in mainnav-out mainnav-lg');
			bindSmallNav();
		}
	...
	init = function(){
		if (!defaultSize) {
			defaultSize = {
				xs : 'mainnav-out',
				sm : jsinit.mainNav.data('sm') || jsinit.mainNav.data('all'),
				md : jsinit.mainNav.data('md') || jsinit.mainNav.data('all'),
				lg : jsinit.mainNav.data('lg') || jsinit.mainNav.data('all')
			}
	...
	methods = {
		'revealToggle' : function(){
			if (!jsinit.container.hasClass('reveal')) jsinit.container.addClass('reveal');
			jsinit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealIn' : function(){
			if (!jsinit.container.hasClass('reveal')) jsinit.container.addClass('reveal');
			jsinit.container.addClass('mainnav-in').removeClass('mainnav-out mainnav-lg mainnav-sm');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealOut' : function(){
			if (!jsinit.container.hasClass('reveal')) jsinit.container.addClass('reveal');
			jsinit.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'slideToggle' : function(){
			if (!jsinit.container.hasClass('slide')) jsinit.container.addClass('slide');
			jsinit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		...
		'pushToggle' : function(){
			jsinit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm');
			if (jsinit.container.hasClass('mainnav-in mainnav-out')) jsinit.container.removeClass('mainnav-in');
			//if (jsinit.container.hasClass('mainnav-in')) //jsinit.container.removeClass('aside-in');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'pushIn' : function(){
		...
		'colExpToggle' : function(){
			if (jsinit.container.hasClass('mainnav-lg mainnav-sm')) jsinit.container.removeClass('mainnav-lg');
			jsinit.container.toggleClass('mainnav-lg mainnav-sm').removeClass('mainnav-in mainnav-out');
			return jsinit.window.trigger('resize');
		},
		'collapse' : function(){
			jsinit.container.addClass('mainnav-sm').removeClass('mainnav-lg mainnav-in mainnav-out');
			updateMethod = 'collapse';
			return jsinit.window.trigger('resize');
		},
		'expand' : function(){
			jsinit.container.removeClass('mainnav-sm mainnav-in mainnav-out').addClass('mainnav-lg');
			return jsinit.window.trigger('resize');
		},
		'togglePosition' : function(){
			jsinit.container.toggleClass('mainnav-fixed');
			jsinit.mainNav.jsinitAffix('update');
		},
		'fixedPosition' : function(){
			jsinit.container.addClass('mainnav-fixed');
			jsinit.mainNav.jsinitAffix('update');
		},
		'staticPosition' : function(){
			jsinit.container.removeClass('mainnav-fixed');
			jsinit.mainNav.jsinitAffix('update');
		},
		'update' : updateNav,
	...
	$.jsinitNav = function(method,complete){
		if (methods[method]){
			if(method == 'colExpToggle' || method == 'expand' || method == 'collapse'){
				if(jsinit.screenSize == 'xs' && method == 'collapse'){
					method = 'pushOut';
				}else if((jsinit.screenSize == 'xs' || jsinit.screenSize == 'sm') && (method=='colExpToggle' || method == 'expand') && jsinit.container.hasClass('mainnav-sm')){
					method = 'pushIn';
				}
			}
	...
	$.fn.isOnScreen = function(){
		var viewport = {
			top : jsinit.window.scrollTop(),
			left : jsinit.window.scrollLeft()
		};
		viewport.right = viewport.left + jsinit.window.width();
		viewport.bottom = viewport.top + jsinit.window.height();
	...
	jsinit.window.on('resizeEnd',updateNav).trigger('resize');
	...
	jsinit.window.on('load', function() {
		...
		if (menu.length) {
			...
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			jsinit.mainNav.find('.nano').nanoScroller({
				preventPageScrolling : true
			});
...
!function ($) {
	...
	var asideMethods = {
		'toggleHideShow' : function(){
			jsinit.container.toggleClass('aside-in');
			jsinit.window.trigger('resize');
			if(jsinit.container.hasClass('aside-in')){
				toggleNav();
			}
		},
		'show' : function(){
			jsinit.container.addClass('aside-in');
			jsinit.window.trigger('resize');
			toggleNav();
		},
		'hide' : function(){
			jsinit.container.removeClass('aside-in');
			jsinit.window.trigger('resize');
		},
		'toggleAlign' : function(){
			jsinit.container.toggleClass('aside-left');
			jsinit.aside.jsinitAffix('update');
		},
		'alignLeft' : function(){
			jsinit.container.addClass('aside-left');
			jsinit.aside.jsinitAffix('update');
		},
		'alignRight' : function(){
			jsinit.container.removeClass('aside-left');
			jsinit.aside.jsinitAffix('update');
		},
		'togglePosition' : function(){
			jsinit.container.toggleClass('aside-fixed');
			jsinit.aside.jsinitAffix('update');
		},
		'fixedPosition' : function(){
			jsinit.container.addClass('aside-fixed');
			jsinit.aside.jsinitAffix('update');
		},
		'staticPosition' : function(){
			jsinit.container.removeClass('aside-fixed');
			jsinit.aside.jsinitAffix('update');
		},
		'toggleTheme' : function(){
			jsinit.container.toggleClass('aside-bright');
		},
		'brightTheme' : function(){
			jsinit.container.addClass('aside-bright');
		},
		'darkTheme' : function(){
			jsinit.container.removeClass('aside-bright');
		}
	},
	toggleNav = function(){
		if(jsinit.container.hasClass('mainnav-in')){
			jsinit.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
		}
	};
	...
	$.jsinitAside = function(method,complete){
	...
	jsinit.window.on('load', function() {
		if(jsinit.aside.length){
			// STYLEABLE SCROLLBARS
			// =================================================================
			// Require nanoScroller
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			jsinit.aside.find('.nano').nanoScroller({
				preventPageScrolling : true,
				alwaysVisible: false
			});
...
!function ($) {
	...
	$.fn.jsinitLanguage = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
...
!function ($) {
	...
	$.fn.jsinitAffix = function(method){
		return this.each(function(){
			var el = $(this), className;

			if (typeof method === 'object' || !method){
				className = method.className;
				el.data('jsinit.af.class', method.className);
			}else if (method == 'update') {
				className = el.data('jsinit.af.class');
			}

			if (jsinit.container.hasClass(className) && !jsinit.container.hasClass('navbar-fixed') ) {
				el.affix({
					offset:{
					top:$('#navbar').outerHeight()
					}
				});
			}else if (!jsinit.container.hasClass(className) || jsinit.container.hasClass('navbar-fixed')) {
				jsinit.window.off(el.attr('id') +'.affix');
	...
	jsinit.window.on('load', function(){
		if (jsinit.mainNav.length) {
			jsinit.mainNav.jsinitAffix({className : 'mainnav-fixed'});
		}

		if (jsinit.aside.length) {
			jsinit.aside.jsinitAffix({className : 'aside-fixed'});
		}
	});
