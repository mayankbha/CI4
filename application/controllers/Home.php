...
class Home extends CI_Controller
{
    ...
    function registration($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == "add_info") {
            ...
            $this->form_validation->set_rules('password1', 'Password', 'required|matches[password2]|min_length[6]|max_length[25]|callback_is_password_strong', array('required' => 'The %s is required.', 'matches' => 'The %s is not matching with %s.', 'is_password_strong' => 'The %s must have at least one capital letter, a number, and one of !, @, $ and of 6-25 characters.'));

            // $this->form_validation->set_rules('is_password_strong', 'Password Should Be Min 6 - 25 characters with numbers & special characters', 'required');

            // $this->form_validation->set_rules('password1', 'Password', 'required|matches[password2]|min_length[6]|max_length[25]|callback_is_password_strong');

            $this->form_validation->set_rules('password2', 'Confirm Password', 'required');

            $this->form_validation->set_rules('terms_check', 'Terms & Conditions', 'required', array('required' => 'You must read and agree with term & condition / privacy policy'));
    ...
    // function to connect remote site
    public function remote_site($site ='',$type=''){

        // echo "<pre>";
        // print_r($type);
        // die();

        $type = rawurldecode(base64_decode($type));
        ...
        if($this->session->userdata('user_id')){
			...
			if($site=='lance') {
				$password  = $data['username'].$email_parts[0];
            ...
            if ($site == 'books') {
                ...
                $sub = "estate";
                $tld = "itbsh.com";
                $controller = "index.php/frontend/";
                
            } else {

            }

            $url = "http://".$sub.".".$tld."/".$controller."/login_via_main";
            ...
            exit;
                ...
                echo '<h1>Error! Your Eye Session is Timed Out Kindly <a href="http://test.com/home/auth/login"> Click Here</a> To Re-login...</h1>';
                exit;
            }
        }
