...
class Admin extends CI_Controller
{
        ...
        if ($para1 == "send") {
            ...
            foreach ($subscribers as $key => $subscriber) {
                if ($subscriber !== '') {
                    ...
                    $user_id = $this->db->get_where('user', array('email' => $subscriber))->row()->user_id;
                    ...
                    $text .='<p><span [removed]="font-weight: bold;">*</span>You can unsubscribe newsletter from your profile, or simply click <a href="https://eyeniversum.com/admin/unsub?data='.base64_encode($user_id.'/'.$subscriber).'">Here</a><br></p>';

                    $this->email_model->newsletter($title, $text, $subscriber, $from);
                }
            }
        } else {
            $page_data['users'] = $this->db->get_where('user', array('email_check' => '1'))->result_array();
    ...
    function unsub($para1 = "")
    {
        ...
        $this->db->update('user', array('email_check' => 0));

        // $this->db->where('user_id', $user_id)->update('user', array('email_check' => 0));

        echo '<h1>Sad To See You Go!</h1><br><h3>You have successfully un-subscribe from newsletter list, you will no longer receive newslleter from Eye Niversum. You can Re Subscribe again anytime by login to your eyeniverum account.</h3><br><h2><b>Thanks For Being A Part Of Eye Niversum</b></h2><br><h3>* Click <a href="https://eyeniversum.com/">Here</a> To Go Back To Eye Niversum Home Page</h3>';
        die();
        // redirect(base_url() . 'admin');
    }
    ...
    function slider($para1 = '', $para2 = '', $para3 = '')
    {
        if ($para1 == 'list') {
            ...
            $data['title']  = $this->input->post('title');
            $data['bg_type']  = $this->input->post('bg_type');

            $data['serial'] = 0;
            $data['status'] = 'ok';

			if($data['bg_type'] == 'bg_color')
			...
			foreach ($style['buttons'] as $btn) {
                if ($this->input->post($btn['name']) !== '') {
                    $buttons[] = array(
                        'name' => $btn['name'],
                        'text' => $this->input->post($btn['name']),
						'btn_url' => $this->input->post($btn['name'] . '_button_link'),
                        'color' => $this->input->post($btn['name'] . '_color'),
                        'background' => $this->input->post($btn['name'] . '_background')
                    );
                }
            }
			foreach ($style['videos'] as $video) {
                if ($this->input->post($video['name']) !== '') {
					$video_url = $this->input->post($video['name']);
			...
			if($video_url != '') {
				...
				// video json data
				$json_result = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=$video_id&key=AIzaSyA_MWVOl_fZx5T_sOox4fN8PZ-C9saABb4");
				...
				$full_slide_style = "data-ls=slidedelay:".$seconds;
				//$replace_full_slide_style = str_replace('slidedelay', 'slidedelay:'.$seconds, $full_slide_style);

				$original_style['full_slide_style'] = $full_slide_style;

				$this->db->where('slider_style_id', $data['style']);
				$this->db->update('slider_style', array('value' => json_encode($original_style)));
			}
			...
			if($video_url != '') {
				...
				//$full_slide_style = $original_style['full_slide_style'];
				$full_slide_style = "data-ls=slidedelay:".$seconds;

				//$replace_full_slide_style = str_replace('slidedelay', 'slidedelay:'.$seconds, $full_slide_style);

				$original_style['full_slide_style'] = $full_slide_style;
				//$original_style['full_slide_style'] = $replace_full_slide_style;
   ...
   /* Manage Frontend Captcha Settings Credentials */
    ...
    function general_settings($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == "disclaimer") {
            ...
            $this->db->update('general_settings', array(
                'value' => $this->input->post('disclaimer')
            ));
            recache();
        }
        if ($para1 == "cookies_policy") {
            $this->db->where('type', "cookies");
            $this->db->update('general_settings', array(
                'value' => $this->input->post('cookies_policy')
            ));
            recache();
        }
        if ($para1 == "about_us") {
